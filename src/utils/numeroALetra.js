...
export default function numeroALetras(cantidad, moneda) {
  var singular;
  var plural;
  var abreviacion;
  ...
  if (cantidad == '0.00' || cantidad == '0') {
    return `CERO ${plural} 00/100 ${abreviacion}`;
  } else {
    ...
    if (numero == 'UN ') {
      return `UN ${singular} ` + ent[1] + `/100 ${abreviacion}`;
    }
    if (numero == 'UN MIL') {
      return `MIL ${plural} ` + ent[1] + `/100 ${abreviacion}`;
    }

    return numero + plural + ' ' + ent[1] + '/100 ' + abreviacion;
  }
  ...
  function unidadesdemillon(
    ...
    var centenasDeMillar = centenasdemillar(
      centenamill,
      decemill,
      unimill,
      centena,
  ...
  function decenasdemillon(
    ...
    var numero =
      decenas(decemillon, unimillon) + 'MILLONES ' + centenasDeMillar;
    return numero;
  }
